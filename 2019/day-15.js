...
function solve(puzzleInput) {
  ...
  const commandsQueue = Object.keys(dirs);
  let o2Coords;
  while (commandsQueue.length) {
    const commands = commandsQueue.shift();
    const coords = commands
      .split(',')
      .reduce(
        (coords, command) => coords.map((coord, i) => coord + dirs[command][i]),
        [0, 0]
      );

    if (coords.join() in map) {
      // skip if seen
      continue;
    }
