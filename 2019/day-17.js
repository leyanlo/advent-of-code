...
function solve1(puzzleInput) {
  const output = intcode(null, puzzleInput);
  debug && console.log(String.fromCharCode(...output));
  const rows = String.fromCharCode(...output).split('\n');
  let sum = 0;
  let pos;
  let dir;
  const scaffold = {};
  ...
  const actions = [];
  while (true) {
    if (scaffold[[pos[0] + dir[0], pos[1] + dir[1]].join()]) {
      actions[actions.length - 1]++;
      pos[0] += dir[0];
      pos[1] += dir[1];
    } else if (
      scaffold[
        [pos[0] + rotate(dir, 'R')[0], pos[1] + rotate(dir, 'R')[1]].join()
      ]
    ) {
      actions.push('R', 1);
      dir = rotate(dir, 'R');
      ...
      scaffold[
        [pos[0] + rotate(dir, 'L')[0], pos[1] + rotate(dir, 'L')[1]].join()
      ]
    ) {
      actions.push('L', 1);
      dir = rotate(dir, 'L');
  ...
  debug && console.log(actions.join());
}

function rotate(dir, turn) {
  const dirs = ['-1,0', '0,1', '1,0', '0,-1'];
  const i = dirs.indexOf(dir.join());
  return dirs[(i + (turn === 'R' ? 1 : -1) + 4) % 4].split(',').map(Number);
}

function solve2(puzzleInput) {
  puzzleInput[0] = 2;
  const routine = `A,A,B,C,A,C,A,B,C,B
R,12,L,8,R,6
