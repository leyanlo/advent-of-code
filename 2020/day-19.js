...
function expandRule(rules, i) {
  let expandedRule = [];
  for (let subRules of rules[i]) {
    let expandedSubRules = [''];
    for (let subRule of subRules) {
      // single char
      if (/^"a|b"$/.test(subRule)) {
        expandedSubRules = [subRule[1]];
        break;
      }

      // sub-rules
      const expandedSubRule = expandRule(rules, subRule);
      expandedSubRules = expandedSubRules
        .map((a) => {
          return expandedSubRule.map((b) => {
            return a + b;
          });
        })
        .flat();
    ...
    expandedRule = [...expandedRule, ...expandedSubRules];
  }
  return expandedRule;
}
...
function solve1(input) {
  ...
  rules = rules.reduce((acc, rule) => {
    const [i, subRules] = rule.split(': ');
    acc[i] = subRules.split(' | ').map((subRule) => subRule.split(' '));
    return acc;
  }, {});

  const rule0 = expandRule(rules, 0);

  const count = messages.reduce((count, r) => count + rule0.includes(r), 0);
  console.log(count);
}
...
function solve2(input) {
  ...
  rules = rules.reduce((acc, rule) => {
    ...
    acc[left] = right.split(' | ').map((subRule) => subRule.split(' '));
    return acc;
  }, {});
  ...
  const count = messages.reduce((count, message) => {
    ...
    let find = rule42.find((rule) => m.startsWith(rule));
    let count42 = 0;
    while (find) {
      count42++;
      m = m.substr(find.length);
      find = rule42.find((rule) => m.startsWith(rule));
    }

    find = rule31.find((rule) => m.endsWith(rule));
    let count31 = 0;
    while (find) {
      count31++;
      m = m.substr(0, m.length - find.length);
      find = rule31.find((rule) => m.endsWith(rule));
    }
    return count + (!m && count42 > count31 && count31 > 0);
  }, 0);
  console.log(count);
}
